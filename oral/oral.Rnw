...
\[
X \overset{MAP}{\rightarrow} \left(S^p\right)^n \overset{ilr}{\rightarrow}    \left(\mathbb{R}^{p-1}\right)^n \overset{\overset{\text{réduction de}} {dimension}}{\rightarrow} \left(R^D\right)^n \rightarrow \text{Apprentissage densité}
\]
\[
\text{densité} \overset{simulation}{\rightarrow} \left(R^D\right)^n \rightarrow
...
\item gain au niveau de l'analyse des échantillons, perte au niveau des OTU (espèces)
