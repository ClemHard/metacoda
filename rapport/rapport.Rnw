...
Le but de ce stage a été de transformer ces données de comptage en données compositionnelles avant de leur appliquer des méthodes d’analyses adéquates (notamment les transformations
...
\end{align*}
\underline{Rm:}\\
La transformation clr est un morphisme de \(\left(S^D, \oplus,\odot\right)\) dans \(\left(\tilde{R}^D,+,*\right)\) avec \(\tilde{R}=\left\lbrace x\in \mathbb{R}^D, \sum_{i=1}^D x_i=0\right\rbrace\).\\
De même la transformation ilr est un morphisme de \(\left(S^D,\oplus,\odot\right)\) dans \(\left(R^{D-1}, +, *\right)\).\\
