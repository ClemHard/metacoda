...
\[
\pi_{jk} = \frac{N_{jk} + \alpha}{N_k + \alpha}
\]
avec $\alpha = 0.1$ un pseudo-comptage faible pour lisser les probabilités et éviter d'avoir des valeurs à $1$. La régularisation adoptée revient à mettre un à priori $\mathcal{D}(\alpha, \alpha)$ sur la probabilité de $0$-inflation. Par la suite, nous modélisons les zéros à l'aide d'une loi de Bernoulli,
