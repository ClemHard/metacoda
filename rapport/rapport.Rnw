...
Exemple: Lors d'un, les personnes interrogées ont eu le choix entre cinq réponses possible A,B,C,D,E.
On souhaiterais savoir si la moyenne géométrique des personnes ayant répondue D,E est plus élevé que celle des personnes ayant répondue A,B,C. Dans ce cas, la matrice binaire adaptée est \(\left(1,1,1,-1,-1\right)\). Une coordonnée ilr inférieur (resp supérieur) à 0 indique la moyenne géométrique de D,E est supérieur (resp inférieur) à celle de A,B,C.
\\
\\
...
Si nous reprenons l'exemple du début de cette partie (avec les quatre compositions), nous remarquons bien que les distances de Aitchison dans le simplexe et les distances euclidiennes après transformation ilr sont belles et bien différentes, alors que dans le simplexe la distance euclidienne est identique (la distance euclidienne n'est donc pas la distance adaptée dans le simplexe).\\
...
\section{Simulateur}
Passons maintenant à la partie simulation de jeux de données. L'objectif est de développer un simulateur capable de générer des jeux de données ressemblant aux jeux de données réels (les caractéristiques des différents groupes des donnée réels doivent être bien simulées).
