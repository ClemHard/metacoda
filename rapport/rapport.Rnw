...
A cela, il est d'ajouter un produit scalaire ainsi qu'une distance et une norme associée.\\
...
Pour centrer des données, il est alors nécessaire d'appliquer la perturbation avec \(g^{-1}\). Appliquer la puissance avec \(totvar\left[X\right]^{-1/2}\) permet de les réduire. 
...
En effet, au lieu de réaliser la transformation puis de centrer, réduire les données, ils centrent puis réduisent (avec les formules définit dans la géométrie de Aitchison) les données avant de réaliser la transformation. Ce qui est équivalent.
...
Dans cette objectif, les feuilles de trois chêne ont été prélevé et séquencé (40 sur chaque arbre). Pour chaque arbre, les  feuilles proviennent de quatre branches différentes (10 pour chaque branche) ; pour chaque feuille plusieurs caractéristiques ont été relevé: sa distance à la base de l'arbre, au tronc de l'arbre et du sol ainsi que son orientation (South west, North East) et enfin son niveau d'infection.\\
Cependant dans le jeu de donnée, seul 116 échantillons de feuilles sont présentes (il en manque quatre), sur chaque échantillon le nombre de 114 OTU est donnée.

L'objectif durant mon stage était de retrouver pour chaque échantillon l'arbre à lequel il appartenait.
...
Dans le cas de certains classificateur tels que le svm, la classification supervise sur certains jeux de données de comptage ne donne pas du tout des résultats concluant (voir chaillou en Annexe \ref{annexe classification supervise}).
...
Le nombre de gaussienne nécessaire à l'estimation de la densité est choisis en minimisant le critère BIC.\\
...
kable(t_liver_500$all$random_forest) 
 ...
 \end{tabular}
\end{center}
Les performances du simulateur sont grandement variable selon le jeu de données. En effet, sur certains jeu de donnée (voir annexe \ref{annexe donnees entier}) les classificateurs peinent à faire la différence entre les données simulées et réels.\\
