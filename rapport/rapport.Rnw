...
Pour Liver et Mach, le nombre d'OTU étant très grand et la matrice de comptage extrêmement creuse, j'ai choisi d'en enlever une partie (ceux présents dans moins de \(5\%\) des échantillons). Les nouvelles dimensions des jeux de données sont ainsi: 155 échantillons sur 1084 OTU pour Mach, 237 échantillons et 533 OTU pour liver. Dans la suite du rapport, seuls les résultats obtenus sur les jeux de données Vacher et Liver seront présentés. Les résultats sur les autres jeux de données sont comparables et présentés en annexe.
...
Dans les grandes lignes, le simulateur apprend la densité des données réelles comme suit. Les données initiales (données de comptage) sont tout d'abord transformées en données compositionnelles (via le MAP), avant d'être envoyées dans $\R^{D-1}$ avec la transformation ilr. Une fois revenu dans un espace euclidien, le simulateur apprend la densité des données à l'aide d'une réduction de dimensions et d'un mélange gaussien dans l'espace réduit. La simulation de nouvelles données peut alors être réalisé en tirant des données suivant le mélange gaussien puis en appliquant les opérations inverses ("augmentation de dimension", \(ilr^{-1}\), etc) pour obtenir un nouveau jeu de données de comptage. Lors du calcul de l'estimateur MAP, un a priori de Dirichlet a été choisi, la transformation inverse consistant à passer d'un point du simplexe à un vecteur de comptages est un tirage suivant une loi multinomiale.
...
\begin{minipage}{0.45\linewidth}
      ...
      \end{minipage}
\end{tabular}
\caption{\label{fig:melange}Exemple d'un mélange de trois gaussiennes univariés (gauche) et bivariés (droites)}
\end{figure}
...
Commençons par estimer la proportion de zéros pour chaque OTU $j$ dans chaque composante gaussienne $k$, notée \(\pi_{jk}\). Cette estimation est faite en attribuant chaque échantillon à son composante gaussienne la plus probable puis en calculant au sein de chaque composante $k$, le nombre total $N_k$ d'échantillons et le nombre $N_{jk}$ d'échantillons dans lesquels l'OTU $j$ est absent. $\pi_{jk}$ est estimée par
\[
...
\centering
 ...
 \multicolumn{3}{c}{\quad \quad \quad \quad \ \ $\downbracefill$} \\
  ...
  \end{array}
\]
\caption{\label{tab:zeo_inflation} Exemple de matrice de zero-inflation.}
\end{table}
Il simule un nouveau jeu de données (tableau de gauche), 3 données appartiennent à la gaussienne rouge et 5 à la gaussienne verte. Lors de la zero inflation, le simulateur va multiplier chaque comptage d'OTU du nouveau jeu de donnée (chaque case du tableau) par une variable de bernoulli de probabilité défini dans la case correspondante (OTU et gaussienne identique) du tableau Tab.~\ref{tab:zero_inflation}. Le résultat final obtenu par le simulateur est alors le jeu de donnée de droite.
\begin{center}
\begin{tabular}{cc}
