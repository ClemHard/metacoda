...
\]
Cet estimateur peut être perçu comme la variance perdue lors de la projection. Le choix de $d$, la dimension de l'espace latent, est toujours une question délicate dans les modèles à variables latentes. Nous avons utilisé ici l'heuristique de pente (du package capushe) pour choisir $d$.

\MM{}{REF capushe, REF ACP probabiliste}
...
MM{}{Le calcul des $\pi_{jk}$ est confus, essaie de le clarifier, par exemple en mettant les formules.}
...
\]
avec $\alpha = ??$ un pseudo-comptage faible pour lisser les probabilités et éviter d'avoir des valeurs à $0$ ou $1$ \MM{}{A vérifier}. La régularisation adoptée revient à mettre un à priori $\mathcal{D}(\alpha, \alpha)$ sur la probabilité de $0$-inflation. Par la suite, nous modélisons les zéros à l'aide d'une loi de Bernoulli,
...
\]
La profondeur de séquencage ($N$ de la multinomiale) à utiliser dans le simulateur est donc \(\hat{N_i}=N_i/\hat{p_i}\).
\newpage
