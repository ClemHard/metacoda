...
L'une des problématiques pour l'étude statistiques des données issu de la métagénomique est due à la différence du nombre d'OTU par échantillon. En effet, les séquenceurs haut débit offre la possibilité de séquencer un grand nombre d'échantillon en même temps, mais ne garantit pas le nombre de séquence (reads) obtenus par échantillon. Le nombre d'OTU obtenu suite au séquencage dans un échantillon peut être par exemple de 1000 tandis que dans un autre il ne peut être que de 500.\\
...
Une classification non supervisé répond bien à cette question, ainsi nous allons utiliser plusieurs algorithmes (k-means, Mélange gaussien, classification hiérarchique) et regarder s'ils ont tendance à regrouper les échantillons des mêmes groupes.\\
...
Dans le cas de Liver, le classificateur a tendance sur les données de comptage à toute les mettre dans le même groupe.\\

 
\subsection{Classification supervise}
Regardons maintenant les performances de classification supervisée par le biais de deux algorithmes qui sont, le randomForest (avec la matrice de confusion) et le support machine vector (leave one out). \\
...
\end{tabular}
\end{center}
Le même phénomène que dans la classification non supervisée est observable. Le classificateur sur les données de comptage a parfois tendance à mettre tous les échantillons dans le même groupe.\\
...
\subsection{Méthode}
La méthode la plus facile à mettre en œuvre est de regarder visuellement si les données simulées ressemble aux données réels, pour ce faire j'ai regarder les projections des données (réels, simulées) sur les premiers axes d'une acp.
...
\end{tabular}
\end{center}
Malheureusement les données considéré comme réels (bien simulés) ne semblent pas plus facile à classer. Les jeux de données où les résultats sur l'ensemble des données simulées étaient déjà très convaincant le sont toujours ; mais les jeux de données où la classification des données simulée était plus compliquée reste compliquée.
\section{Conclusion}
