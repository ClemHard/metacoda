...
\end{center}}
\def\myauthor{\begin{center}
 Clément Hardy\\ encadré par Mahendra Mariadassou (MaIAGE), Magali Berland (MGP) \end{center}}	

\def\mydate{\begin{center}
 27 Août 2018\end{center}}
...
J'ai réalisé mon stage à l'Inra de Jouy en Josas dans l'unité "Mathématiques et Informatique Appliquées du Génome à l'Environnement" (MaIAGE) (\url{http://maiage.jouy.inra.fr/}).
...
J'ai été accueillis au sein de l'équipe StatInfOmics.
\subsection{Equipe}
...
La métagénomique est une méthode d'analyse du contenu génétique d'un échantillon issu d'un milieu complexe, c’est à dire qui abrite de nombreuses espèces, tel que le microbiote intestinal. La métagénomique, à l'inverse de la génomique, analyse le génome de l'ensemble des espèces présentes dans un échantillon. Cette évolution s'est faite grâce aux nouvelles méthodes de séquencage dit de séquencage haut débit ainsi qu'a la bioinformatique.\\
Deux méthodes de séquençage existe, l'une dite "shotgun sequencing" consiste à séquencé la totalité du génome. Pour cela, les séquences d'ADN sont d'abord découpé en petit fragment de longueur prédéfinie ; ceux si sont ensuite séquencé individuellement.\\
Les séquences sont par la suite réassemblé informatiquement dans le but de reconstruire le génome et de lui assigner une espèce bactérienne.\\
L'autre méthode ne séquence pas l'ensemble du génome mais seulement un gêne marqueur l'est: l'ARN 16S.
Les séquences (lectures) obtenues suite au séquencage sont assignées à une espèce bactérienne(l'étape de reconstruction du génome n'est pas à faire).\\

 Une fois le séquencage effectué, la table de comptage des OTUS est contruite (un OTU, ou Operational Taxonomic
Unit, est un ensemble de lectures qui se ressemblent et est souvent utilisé comme une espèce bactérienne hypothétique). Dans le cas du séquencage shotgun, le terme employé n'est pas OTU mais MGS, pour simplifier l'écriture MGS sera remplacé par OTU dans la suite du rapport.\\
La table de comptage des OTUS est un tableau contenant le nombre de séquences par OTU observés dans chaque échantillon.

\begin{figure}[H]
\input{tikz_figures/otu_table}
\caption{Tables des OTU}
\end{figure}

L'une des particularité de la table des OTU pour la métagénomique est qu'elle contient énormément de zéro. En effet, un OTU qui est présent dans un seul échantillon apparaîtra avec un comptage nul dans les autres échantillons. Les OTUs présents dans chaque échantillon pouvant être très différents, il y a ainsi énormément de zéro (de quelque dizaine à centaines de milliers de zéro).

\subsection{Problématique des données de comptage}
L'une des problématiques pour l'étude statistique des données issues de la métagénomique est due à la différence du nombre de lectures par échantillon. En effet, les séquenceurs hauts débits offrent la possibilité de séquencer un grand nombre d'échantillons en même temps, mais ne garantit pas le nombre de séquences (profondeur de séquençage) obtenues par échantillon. Le nombre de lectures obtenus suite au séquençage dans un échantillon peut être par exemple de 1000 tandis que dans un autre il ne peut être que de 500.
...
Le but de ce stage est de transformer ces données de comptage en données compositionnelles avant de leur appliquer des méthodes d’analyses adéquates (notamment les transformations
log-ratio) pour les projeter dans un espace euclidien usuel et apprendre la densité des données dans cet espace, à l’aide d’un modèle probabiliste. \\
...
\]
 ...
 En notant \(\kappa\) la somme des composantes souhaité, la closure est définie par:
 \[
C(\kappa,x)=\left(\left[\frac{\kappa*x_1}{\sum_{i=1}^Dx_i},\frac{\kappa*x_2}{\sum_{i=1}^Dx_i},...,\frac{\kappa*x_D}{\sum_{i=1}^Dx_i}     \right]\right)
\]
Dans la suite du rapport sauf mention contraire la notation \(C(x)\) correspondra à \(C(1,x)\).
\vspace*{0.3cm}
...
\vspace*{0.3cm}
\newline
La représentation graphique d'une donnée compositionnelle à trois composantes se fait grâce à un diagramme ternaire.\\
Un diagramme ternaire est un triangle équilatéral ; pour une composition
...
La multiplication, l'addition sont de même inapproprié ; la multiplication d'une composition par un scalaire ne permet pas de rester dans le simplexe (la valeur du \(\kappa\) n'est pas conservée).\\
Pour toute ces raisons, nous avons besoin de définir une nouvelle géométrie. Pour d'autres raisons tel que les régions de confiances de composition aléatoires voir (\cite{Lecture})
\newline
\vspace*{0.3cm}
\newline
...
Il devient ainsi possible de définir des objets géométriques telles que les lignes compositionnelles de direction x et passant par x0 (droite dans l'espace euclidien): \(y=x_0 \oplus \left(\alpha\odot x\right)\), avec \(x,x_0 \in S^D\) et \(\alpha \in \mathbb{R}\). Ce qui donne graphiquement pour D=3,
...
\begin{minipage}{\linewidth}
      ...
      \begin{minipage}{0.45\linewidth}
          ...
          <<fig.height=4.5>>=
              ...
              @
              \caption{Avant centrage et réduction}
          \end{figure}
      \end{minipage}
      ...
      \begin{minipage}{0.45\linewidth}
          ...
          par(mar=c(rep(0,4)))
              ...
              @
              \caption{Après centrage et réduction}
          \end{figure}
      \end{minipage}
  \end{minipage}\\
...
\]
 le système sera alors lié. L'alr résout ce problème en prenant une composante comme dénominateur (une des coordonnées est ainsi nulle).
Cependant, cette transformation est grandement dépendante de la composante choisie comme dénominateur. De plus, cette transformation ne préserve pas les distances, c'est à dire
\[
...
x_1=\left[0.1,0.4,0.5\right],\ x_2=\left[0.2,0.3,0.5\right],\ x_3=\left[0.4,0.4,0.2\right],\ x_4=\left[0.5,0.3,0.2\right]
\) 
et en calculant les distances de Aitchison entre elles dans le simplexe et les distances euclidiennes après la transformation alr,
...
\vspace*{0.3cm}
La transformation centred log-ratio (clr) n'a pas ces deux problèmes.
...
\]
avec \(\Psi\) l'image \(clr\) de la base. La méthode pour obtenir  l'image de la base de référence est expliquée ci dessous.\\
\\
La transformation ilr permet de résoudre les problématiques des autres transformations, elle préserve les distances et ne génère pas une matrice de covariance liée. Pour cela, elle utilise un type de coordonnée particulier nommé balance.\\

Image de la base de référence:\\
Prenons comme système générateur de \(\mathbb{R}^{D-1}\), les vecteurs canoniques
\[
e_i=(\underset{i-1}{\underbrace{0,...,0}},1,0,...,0),\quad i=1,...D-1
\]
en appliquant \(clr^{-1}\) à ces vecteurs, on obtient un système générateur du simplexe,
\[
W_i=C([\underset{i-1}{\underbrace{1,...,1}},e,1,...1]),\quad i=1,...D-1
\]

En prenant l'image \(clr\) de ces vecteurs (le résultat n'est pas les vecteurs canoniques étant donnée que la closure a été appliqué entre les deux transformations, \(clr^{-1}\) et \(clr\)), puis en utilisant Gram-Schmidt  pour l'orthogonaliser, on obtient la matrice \(\Psi\).\\
...
D'où le nom de balance. Une coordonnée ilr négative (balance négative) signifie que la moyenne géométrique des composantes du dénominateur a plus de poids que la moyenne géométrique des composantes du numérateur.\\
Exemple: Lors de plusieurs sondages, les personnes interrogées ont eu le choix entre cinq réponses possible A,B,C,D,E.
On souhaiterais savoir si la moyenne géométrique des personnes ayant répondue D,E est plus élevé que celle des personnes ayant répondue A,B,C. Dans ce cas, la matrice binaire adaptée est \(\left(1,1,1,-1,-1\right)\).
\\
\\
...
\end{align*}
\underline{Rm:}\\
La transformation clr est un morphisme de \(S^D\) dans \(\tilde{X}^D\) avec \(\tilde{X}=\left\lbrace x\in \mathbb{R}^D, \sum_{i=1}^D x_i=0\right\rbrace\).\\
De même la transformation ilr est un morphisme de \(S^D\) dans \(R^{D-1}\).\\

Si nous reprenons l'exemple du début de cette partie (avec les quatre compositions), nous remarquons bien que les distances de Aitchison dans le simplexe et les distances euclidiennes après transformation ilr sont belles et bien différentes.\\
...
Aitchison J redéfinie l'acp dans les données compositionnelles.\\
En effet, le centrage et la réduction des données ne se fait pas après la transformation (clr, ilr) mais avant celle ci (avec les formules définies dans la géométrie de Aitchison) ; ce qui est équivalent (ces transformation sont des morphismes).

\subsection{Maximum a posteriori}
Nos jeux de données étant des jeux de données de comptage, de nombreux zéros sont présents comme expliqué précédemment ; les transformations logarithmiques (ilr, clr) ne peuvent ainsi pas être directement appliquées ; il est nécessaire d'effectuer un lissage lors du passage des données de comptage
aux proportions.
...
En mettant une distribution a priori sur les proportions \(\theta\) et en utilisant le théorème de Bayes, nous avons
...
Dans notre cas, nous allons prendre un a priori de Dirichlet, plus précisément \(Dir\left(\alpha\right)\) avec \(a=\left(1,...,1,\right)\) ce qui correspond à une loi uniforme sur les compositions. Il en découle l'estimateur suivant:
\[
MAP(\theta)=\underset{\therefore}{argmax}\left(\theta|X\right)=\frac{X_i+1}{\sum_{i=1}^D\left(X_i+1\right)}
\]
...
Il s'agit d'un jeu de données d'origine alimentaire, il provient d'une étude sur les communautés bactériennes qui se développent lorsque la nourriture périme.\\
Pour cela, quatre types de viande : boeuf hache, veau hache, lardons, saucisses de volaille et quatre types de produits de la mer: saumon fumé, crevettes, filet de saumon et filet de cabillaud ont été échantillonnés.\\
...
L'objectif de cette étude était de caractériser le microbiote intestinal lié à la cirrhose du foie, pour cela le microbiote de 114 patients sains et 123 patients atteint de cirrhose ont été séquencés. Pour chaque patients, plusieurs caractéristiques ont été relevé tels que l'indice de masse corporel, un indice de coagulation du sang, des tests du bon fonctionnement des reins...\\
Dans le cadre de l'étude, 1529 OTU ont été conservé.\\
Les deux groupes qui sont malades ou non malades ont été les groupes étudiées durant mon stage.
...
Les classificateurs ont tendance sur les données de comptage à toutes les mettre dans le même groupe. La séparation entre les groupes n'a pas l'air toujours très franche comme dans le cas de Liver (un grand d'échantillon est mal classé) aussi bien pour les données de comptage que pour les données compositionnelles.\\

 
\subsection{Classification supervise}\label{Classification supervise}
Regardons si des classificateurs supervisées permettent de mieux faire la différence entre les groupes (notamment sur les jeux de données où une classification non supervisée ne donne pas de bon résultats). Pour cela une classification supervisée a été réalisé par le biais de trois algorithmes qui sont, le randomForest (avec la matrice de confusion), le support machine vector et le k nearest neighbors (en faisant pour les deux derniers une 10-fold cross validation). \\
...
\end{tabular}
\end{center}
Les meilleurs performances des classificateurs sur les données compositionnelles que sur les données de comptage semblent être confirmé, les classifications sur les données compositionnelles (après transformation ilr) sont dans le pire des cas identique à ceux des classification sur les données de comptage.\\
 Malheureusement, dans le cas de certains classificateurs tel que le svm, la classification supervisée sur certains jeux de données de comptage ne donne pas du tout des résultats concluants (voir chaillou en Annexe \ref{annexe classification supervise}).\\
...
\section{Simulateur}
Passons maintenant à la partie simulation de jeux de données. L'objectif est de développer un simulateur capable de générer des jeux de données ressemblant aux jeux de données réels (les caractéristiques des différents groupes des donnée réels sont bien simulées).
Pour ce faire, les données initial (données de comptage) sont tout d'abord transformé en données compositionnelles (via le MAP) puis la transformation ilr leurs est appliquées.\\
Une fois ceux ci fait, le simulateur apprend la densité des données à l'aide d'un mélange gaussien (une réduction de dimension est au préalable nécessaire). La simulation de nouvelles données peut ainsi être réalisé; les opérations inverses sont alors effectué ("augmentation de dimension", \(ilr^{-1}\)....) pour obtenir un nouveau jeu de données de comptage.
Comme lors du calcul de l'estimateur MAP, un a priori de dirichlet a été choisis, la transformation inverse au MAP est alors une multinomiale.\\
...
Les jeux de données contiennent un nombre important de zéros, la modélisation faite par un mélange gaussien ne parvient pas à bien prendre en compte tous ces zéros. Pour résoudre ce problème, nous allons modéliser cette particularité indépendamment. \\
...
Un autre aspect de la performance du simulateur est sa capacité à simuler les caractéristiques de chaque groupe (les différents arbres dans le cas de vacher ; malade, non malade pour Liver) d'un jeu de données ; une donnée simulée ne doit pas avoir les caractéristiques de plusieurs groupes.\\
