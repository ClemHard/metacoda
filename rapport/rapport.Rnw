...
La métagénomique est une méthode de séquencage et d'analyse du contenu génétique d'un échantillon issu d'un milieu complexe (telle que le microbiote intestinal). La métagénomique à l'inverse de la génomique séquence le génome de l'ensemble des espèces présent dans un échantillon. Cette évolution a pus être réalisé grâce aux nouvelles méthodes de séquencage dit de séquencage haut débit ainsi qu'a la bioinformatique.\\
En bactériologie, le génome n'est pas complètement séquencé mais seulement un gêne l'est, l'ARN 16S.\\
Les séquences (reads) obtenus suite au séquencage sont assignées à une espèce bactérienne. Une fois cette étape effectué, la table de comptage des OTUS est contruite (un OTU est un ensemble de bactérie qui se ressemble).\\
La table de comptage des OTUS n'est autre qu'un tableau contenant le nombre de séquence par OTU observé dans chaque échantillon.
...
L'une des problématiques pour l'étude statistiques des données issu de la métagénomique est due à la différence du nombre d'OTU par échantillon. En effet, les séquenceurs haut débit offre la possibilité de séquencer un grand nombre d'échantillon en même temps, mais ne garantit pas le nombre de séquence (profondeur de séquençage) obtenus par échantillon. Le nombre d'OTU obtenu suite au séquencage dans un échantillon peut être par exemple de 1000 tandis que dans un autre il ne peut être que de 500.\\
La comparaison direct des échantillons est donc impossible.\\
Le passage aux données compositionnelles est une méthode de résolution possible de cette problématique.\\

\subsection{Objectif du stage}
Le but de ce stage est de transformer ces données de comptage en données compositionnles et d'utiliser les méthodes d'analyse associée à ces données pour pouvoir leurs appliquer un modèle probabiliste. \\
Ce modèle probabiliste a ensuite servit à développer un simulateur de nouvelles données.
...
\(X=\left[x_1, x_2, x_3\right]\), les composantes sont la distance entre sa  représentation graphique et un des côtes du triangle, comme montrer dans le graphique suivant


\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.5]{ternarydiagram.png}
\end{center}
...
\\
\\
Commençons par définir les opération de bases:\\
...
Il devient ainsi possible de définir des objets géométriques tels que les lignes compositionnels (droite dans l'espace euclidien): \(y=x_0 \oplus \left(\alpha\odot x\right)\), avec \(x,x_0\in S^D\) et \(\alpha in \mathbb{R}\). Ce qui donne graphiquement pour D=3,


\begin{figure}
\begin{center}
\includegraphics[scale=0.5]{ligne_compositionnel}
\caption{Lignes compositionnels}
\end{center}
\end{figure}


Les mesures statistique tels que la moyenne et la variance doivent être elles aussi redéfinit.\\
...
\\
La matrice de covariance quant à elle est définit par
\[
T=\begin{pmatrix}
t_{11} & t_{12} & ... & t_{1D} \\
...  & ... & ... & ...\\
...
\]
ce qui permet de définir la variance total \[ totvar\left[X\right]=\frac{1}{2D}\sum_{i=1}^D\sum_{j=1}^Dt_{ij} \] \\
...
Tout d'abord, remarquons que la valeur de \(\kappa\)(somme des composantes) d'une composition n'a pas fondamentalement une grande importance. En effet, l'étude de données qu'elles soient exprimer en pourcentage ou en proportion ne change rien. \\
...
\\
\\
Ces transformations permettent de passer dans un espace euclidien et de retrouver les formes géométrique habituels.\\
...
\begin{minipage}{\linewidth}
      ...
      \begin{minipage}{0.35\linewidth}
          ...
          \captionsetup{justification=centering}
              \includegraphics[width=\linewidth]{ilr_avant.png}
      ...
      \begin{minipage}{0.55\linewidth}
          \begin{figure}[H]
          \captionsetup{justification=centering}
              \includegraphics[width=\linewidth]{ilr_apres.png}
...
L'ACP est une méthode de réduction de dimension couramment utilisé pour synthétiser l'information.\\
Dans "Lecture Notes on Compositionnal Data Analysis", ils redéfinissent l'acp dans les données compositionnelle.\\
En effet, au lieu de réaliser la transformation puis de centrer et réduire les données, ils les centrent et les réduisent (avec les formules définit dans la géométrie de Aitchison) avant de réaliser la transformation ; ce qui est équivalent.
...
Cinq jeux de données sont mis à ma disposition qui sont Chaillou, Mach, Liver, Ravel ainsi que Vacher. Tous ces jeux de données se présentent sous la forme de table de comptage d'OTU.\\
...
Pour chaque type de nourriture huit échantillons (de lot différents) sont présent dans ce jeu de donnée. 508 OTUs ont été conservé lors de l'étude.\\
...
Les excréments de 31 porcelets ont été séquencés à cinq périodes de leur vie (14, 36, 48, 60 et 70 jours) ; à chaque séquençage le sevrage ou non de l'animal a été relevé. Le jeu de donnée contient ainsi 155 échantillons, le nombre d'OTU quant à lui est 4031.\\
...
Pour répondre à cette question, ils ont séquencé le microbiote vaginal de 394 femmes provenant de quatre catégories ethniques différentes (Asiatiques, blanches, noires et hispanique), 247 OTU ont été gardé pour réaliser cette étude. Sur chaque échantillon (un pour chaque femme), le nugent score a été réalisé, il s'agit d'une méthode de diagnostic de la vaginose bactérienne.\\
Selon le résultat de ce test, les échantillons ont été classé en 3 catégories (low, intermediate, high).
...
L'objectif de l'étude d'où provienne les données était de trouver les interactions les plus probables entre un champignon responsable de l'oidium du chêne (Erysiphe alphitoides) et les autres espèce de microorganisme présent sur les feuilles de chêne (Erysiphe alphitoides).\\
Dans cette objectif, les feuilles de trois chêne ont été prélevé et séquencé (40 sur chaque arbre). Pour chaque arbre, les  feuilles proviennent de quatre branches différentes (10 pour chaque branche) ; pour chaque feuille plusieurs caractéristiques ont été relevé: sa distance à la base de l'arbre, sa distance au tronc de l'arbre et sa distance par rapport au sol ainsi que son orientation (South west, North East) et enfin son niveau d'infection.\\
Cependant dans le jeu de donnée, seul 116 échantillons de feuilles sont présents (il en manque quatre), sur chaque échantillon le nombre de 114 OTU est donnée.

L'objectif durant mon stage était de retrouver pour chaque échantillon l'arbre auquel il appartenait.



Pour Liver, Mach, le nombre d'OTU étant très grand, j'ai choisis d'en enlever une partie (ceux présent dans moins de \(5\%\) des échantillons).\\
...
Pour avoir un premier aperçus des jeux de données, faisons une acp sur les données compositionnelles et regardons la projections des échantillons sur les deux premiers axes.
...
Pour mach la séparation des échantillouns issus d'animaux sevré et non sevré est très franche. Le seul échantillon mal situé n'est autre qu'un échantillon issu d'un animal mal sevré.\\
...
Une classification non supervisé répond bien à cette question, ainsi j'ai utilisé plusieurs algorithmes (k-means, Mélange gaussien, classification hiérarchique) et regarder s'ils ont tendance à regrouper les échantillons des mêmes groupes.\\
Pour montrer l'intérêt de passer par les données compositionnelles pour la classification, j'ai comparer les résultats d'une classification effectuée sur les données initiales (de comptage) et les données compositionnelles (après transformation \(ilr\)).\\
...
Les classificateur ont tendance sur les données de comptage à toute les mettre dans le même groupe.
...
\end{tabular}
\end{center}
Dans le cas de certains classificateur tels que le svm, la classification supervisée sur certains jeux de données de comptage ne donne pas du tout des résultats concluant (voir chaillou en Annexe \ref{annexe classification supervise}).\\
Que se soit pour la classification supervisée ou non supervisée, les résultats sont meilleurs sur les données compositionnelles (après transformation ilr) que sur les données initial. Ils semblent donc avoir un intérêt à passer par les données compositionnelles pour la classification.
...
\subsection{Réduction de dimension}
Les jeux de données utilisé dans le cadre du simulateur ont un très grand nombre de variable (grande dimension); une réduction de dimension s'impose.\\
...
La dimension de l'espace latent a été choisis à l'aide de l'heuristique de pente (grâce au package capushe).\\
...
Voici des exemples de mélange gaussien:
  ...
  \begin{minipage}{\linewidth}
      ...
      \begin{minipage}{0.45\linewidth}
          \begin{figure}[H]
          \captionsetup{justification=centering}
              \includegraphics[width=\linewidth]{melangegaussien2D.png}
      ...
      \end{minipage}
  \end{minipage}\\
Pour le simulateur, le nombre de gaussienne nécessaire à l'estimation de la densité est choisis en minimisant le critère BIC.\\
...
\subsection{Méthode}
La méthode la plus facile à mettre en œuvre est de regarder visuellement si les données simulées ressemblent aux données réels, pour ce faire j'ai regarder les projections des données (réels, simulées) sur les premiers axes d'une acp.
...
Globalement, les données simulées se mélange bien avec les données réels.
La première impression sur les performances du simulateur sont positives.\\
\\
Cependant cette méthode n'est pas forcément la plus efficace. En effet, certain OTU peuvent être très mal simulée sans que pour autant nous puissions faire la différence visuellement. Par exemple, un OTU absent de tous les échantillons réels mais présent en de très faible proportions dans les échantillons simulées n'aura aucune incidence sur une acp, il n'est pas pour autant bien simulé.\\
\\
Pour avoir des résultats plus fiable, l'étude de l'efficacité du simulateur est confiée à des classificateurs.\\
L'objectif est de regarder si un classificateur parvient à faire la différence entre les données réels et les données simulées.\\
Dans cette objectif, deux nouveaux jeux de données sont simulés, l'un d'entre eux servira de jeux d'entrainement (avec les données réels) à un classificateur, l'autre jeu de données servira quant à lui de test. Une petite partie des données réels ne sont pas incluse dans le jeu d'entrainement, elles serviront elles aussi de test.\\
...
kable(t_liver_500$all$random_forest) 
 ...
 \end{tabular}
\end{center}
Les performances du simulateur sont grandement variable selon le jeu de données. En effet, sur certains jeu de donnée les classificateurs peinent à faire la différence entre les données simulées et réels.\\
A l'inverse sur Liver, le random Forest fait aisément la différence, cependant le kNN lui le fait moins facilement. Ainsi certaines caractéristiques des données Liver doivent être mal simulées (ce qui permet au random Forest de faire la différence), mais les données simulées doivent quant même relativement bien se mélanger aux données réels (comme le montre le kNN).

\subsection{Classification}
Un autre aspect de la performance du simulateur est sa capacité à simuler les caractéristiques de chaque groupe d'un jeu de donnée ; une donnée simulée ne doit pas avoir les caractéristiques de plusieurs groupes.\\
Pour étudier cette aspect, la simulation ne se fera plus sur l'ensemble du jeux de données mais séparément sur chaque groupe du jeu de donnée.\\
Les groupes des données simulées seront alors connu et nous pourrons vérifier si un classificateur retrouve bien ses groupes.\\
Voici un schéma illustratif de la méthode \\
Soit \(1,2,...,k\) les groupes du jeu de donnée (ex: pour mach k=2, sevre, non sevre)
\begin{center}
\begin{tikzpicture}[node distance=2cm, auto]
     \node[cloud] (X1) {\(X_1\)};
...
\end{tabular}
\end{center}
La classification supervisée des données simulées sont globalement bonne. Les résultats sont même quasiment comparables aux résultats de la validation croisée sur les données réels (pour le random Forest). Ainsi même si une partie des échantillons simulés est malheureusement "mal simulé" (notamment dans le cas du jeu de données liver); les caractéristiques des groupes restent quant à eux relativement bien simulés. \\


Pour vérifier que les données considéré comme bien "simulées" sont facilement classable dans un groupe (et ainsi relativement proche des données réels de leur groupe respectif), nous allons conserver uniquement ces données puis faire comme précédemment en faisant une classification supervisée et en vérifiant leur bonne classification.
...
Les données considéré comme réels (bien simulés) ne semblent pas plus facile, ni plus difficile à classer que dans le cas précédent. Les jeux de données où les résultats sur l'ensemble des données simulées étaient déjà très convaincant le sont toujours ; mais les jeux de données où la classification des données simulée était plus compliquée reste compliquée (le classificateur kNN pour liver).\\
Cependant sur ces jeux de données, la validation croisée (Classification supervisée) sur les données réels ne permet pas non plus d'obtenir de bon résultats, la problématique ne semble donc pas venir des données simulées.\\


\section{Conclusion}
L'intérêt des données compositionnelles est de pouvoir s'affranchir de certaines contraintes technique comme la profondeur de séquencage.\\
L'utilisation des transformations des données compositionnelles pour repasser dans un espace euclidien s'est révélé pertinent sur les jeux de données utilisé durant mon stage, les résultats des différentes classification étant au moins aussi bonne que sur les données initiales.\\
De même les résultats obtenue sur les données simulées sont encourageants.\\
Les caractéristiques des différents groupes des jeux de données semblent relativement bien simulées. Cependant, les résultats sont très variables selon les jeux de données. Des tests sont un plus grand nombre de jeux de données serait nécessaire pour étudier la robustesse du simulateur.
\section{Annexe}
\appendix
