...
La métagénomique est une méthode d'analyse du contenu génétique d'un échantillon issu d'un milieu complexe (tel que le microbiote intestinal). La métagénomique, à l'inverse de la génomique, analyse le génome de l'ensemble des espèces présentes dans un échantillon. Cette évolution s'est faite grâce aux nouvelles méthodes de séquencage dit de séquencage haut débit ainsi qu'a la bioinformatique.\\
...
L'autre méthode ne séquence pas l'ensemble du génome mais seulement l'ARN 16S (un gêne).
Les séquences obtenues suite au séquencage sont assignées à une espèce bactérienne(l'étape de reconstruction du génome n'est pas à faire).\\

 Une fois le séquencage effectué, la table de comptage des OTUS est contruite (un OTU est un ensemble de bactéries regroupées selon la similarité de leur séquence d'ADN). Dans le cas du séquencage shotgun, le terme employé n'est pas OTU mais MGS, pour simplifier l'écriture MGS sera remplacé par OTU dans la suite du rapport.\\
...
L'une des particularité de la table des OTU pour la métagénomique est qu'elle contient énormément de zéro. En effet, un OTU qui est présent dans un seul échantillon sera quant même comptabilisé dans les autres échantillons. La diversité des OTU dans chaque échantillon pouvant être très différente, il y a ainsi énormément de zéro.

\subsection{Problématique des données de comptage}
L'une des problématiques pour l'étude statistique des données issues de la métagénomique est due à la différence du nombre d'OTU par échantillon. En effet, les séquenceurs hauts débits offrent la possibilité de séquencer un grand nombre d'échantillons en même temps, mais ne garantit pas le nombre de séquences (profondeur de séquençage) obtenues par échantillon. Le nombre d'OTU obtenus suite au séquençage dans un échantillon peut être par exemple de 1000 tandis que dans un autre il ne peut être que de 500.
...
Cinq jeux de données sont mis à ma disposition qui sont Chaillou\cite{Chaillou}, Mach\cite{Mach}, Liver\cite{Liver}, Ravel\cite{Ravel} ainsi que Vacher\cite{Vacher}. Tous ces jeux de données se présentent sous la forme de table de comptage d'OTU.\\
